...
class LoginView(ft.Container):
    ...
    async def login_with_key(self, e):
        ...
        Проверяет ключ через API: если ключ валидный - генерируется
        4-значный PIN, ключ и хэш PIN сохраняются в базу, выполняется вход.

        При неположительном балансе вход разрешен с предупреждением:
        чат работает на бесплатных моделях `:free`, а низкий баланс
        отслеживается уведомлениями (задание 1).
        ...
        # Ключ валиден, но баланс неположительный - предупреждаем,
        # но разрешаем вход: чат работает на бесплатных моделях `:free`,
        # а красный баланс виден в интерфейсе чата
        if not validation["balance_positive"]:
            self.show_status(
                f"Баланс ключа ${validation['balance']:.2f} - "
                "вход разрешен, чат работает на бесплатных моделях"
            )
